...
namespace KCRV_Statistics.Model.GraphicsShell
{
    public class GraphicsSketchers
    {
        ...
        public static List<TextLabelEntity> GetSortedLabels(List<RegularData> Data)
        {
            try
            {
                ...
                // Определение "цены" деления на координатной плоскости КАНВАСА под имеющиеся данные координаты
                var kx = (GraphicsShellConfiguration.InternalCanvasWidth)
                    / Math.Sqrt(Math.Pow(SortedData.Count() - 1, 2));

                // Шаг по оси Х внешнего канваса
                var xStep = Convert.ToInt32(
                    Math.Floor(Convert.ToDecimal(SortedData.Count() / GraphicsShellConfiguration.MaxXLabelCount)) + 1
                );

                // Определение координат подписей номеров лабораторий на канвасе
                int counter = 0;
                for (double x = 0; x <= SortedData.Count - 1; x += xStep)
                {
                    // Текущая координата Х
                    var XCoord = Convert.ToInt32(x * kx);
                    ...
                    Result.Add(
                        new TextLabelEntity
                        {
                            X = XCoord + XFieldInitPoints + GraphicsShellConfiguration.LabelStartCoordinate
                                - GraphicsShellConfiguration.PointRadius / 2,
                            Y = YFieldInitPoints / 4,
                            Content = SortedData[counter].LaboratoryNumber.ToString()
                        }
                    );
                    counter++;
                }

                double value_min = SortedData.Min(x => x.Value - x.Uncertanity);
                double value_max = SortedData.Max(x => x.Value + x.Uncertanity);
        ...
        /// </summary>
        public static List<LineGraphicsEntity> GetUncertanityLines(List<RegularData> Data)
        {
            try
            {
                ...
                var min = Data.Min(x => x.Value - x.Uncertanity);
                var max = Data.Max(x => x.Value + x.Uncertanity);

                // Определение цены одного деления на отображаемой оси
                var kx = (GraphicsShellConfiguration.InternalCanvasWidth)
                    / Math.Sqrt(Math.Pow(Data.Count - 1, 2));
                var ky = (GraphicsShellConfiguration.InternalCanvasHeight)
                ...
                foreach (var Item in SortedData)
                {
                    ...
                    var X = Convert.ToInt32((counter - 1) * kx) + GraphicsShellConfiguration.PointRadius / 2;
                    counter++;
